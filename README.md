...
- **Wiring brings the data; Paper still owns the labels** — a reused component carries its own field names and badges along with the value you wanted; only the values come from the real data source, the structure and labels stay Paper's. Direction B runs again after wiring, against the live page, because a static pass has no prices or badges to leak.
- **Content count is an unavoidable state** — how many photos, chips or rows a listing gets is decided by real data exactly like viewport width is decided by the window. A layout rule is derived for the realistic range of counts, not just the artboard's one sample count.
- **A migrated control may lead off the migrated page** — a correct destination outside this wave's scope reads as broken to a reviewer with no way to tell "left migrated scope on purpose" from "actually broken." Name it in the review request.
