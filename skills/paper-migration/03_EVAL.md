...
| 22 | **Class name transferred, value silently different** | `get_jsx(format: "tailwind")` returns lookups into *Paper's* theme, not values. `px-7` meant 64px in the design file and 28px in the app; the class copied over cleanly, compiled, linted, and moved every section on the page. Invisible per element, global in effect. Extract with `inline-styles`, or run the token-parity table in [01_DISCOVERY.md](01_DISCOVERY.md) **before** transferring markup. |
| 23 | **Transfer and wiring done in one pass** | Structure was never proven on its own, so after data went in, "this node is missing" and "this field is empty" became the same observation — and the agent classified real losses as data gaps. Transfer 1:1 with the artboard's static text, checkpoint the node inventory and geometry, *then* wire. See [02_IMPLEMENTATION.md](02_IMPLEMENTATION.md) §Two passes. |
